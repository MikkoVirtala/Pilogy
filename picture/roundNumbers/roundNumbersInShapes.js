  ...
  for (let i = 0; i < shapes.length; i++) {
    ...
    if (shape.attachmentPoints) {
      ...
      for (let j = 0; j < attachmentPoints.length; j++) {
        ...
        attachmentPoint[0] = roundNumber(attachmentPoint[0])
        attachmentPoint[1] = roundNumber(attachmentPoint[1])
      }
    }

    if (shape.pointToAttach) {
      const pointToAttach = shape.pointToAttach
      pointToAttach[0] = roundNumber(pointToAttach[0])
      pointToAttach[1] = roundNumber(pointToAttach[1])
    }
